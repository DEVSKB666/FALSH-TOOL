...
import { useHistory, sha256Hex, type HistoryOp, type HistoryStatus } from "@/lib/history";
...
const OPERATIONS: OpDef[] = [
  ...
  },
  {
    ...
    icon: Eraser, tone: "red", danger: true,
...
const TONE_CLASSES: Record<OpDef["tone"], { bg: string; ring: string; icon: string; glow: string }> = {
  blue:   { bg: "bg-sky-500/10",    ring: "ring-sky-500/30",    icon: "text-sky-400",    glow: "shadow-[0_0_30px_rgba(14,165,233,0.18)]" },
  amber:  { bg: "bg-amber-500/10",  ring: "ring-amber-500/30",  icon: "text-amber-400",  glow: "shadow-[0_0_30px_rgba(245,158,11,0.18)]" },
  red:    { bg: "bg-red-500/10",    ring: "ring-red-500/30",    icon: "text-red-400",    glow: "shadow-[0_0_30px_rgba(239,68,68,0.22)]" },
  purple: { bg: "bg-purple-500/10", ring: "ring-purple-500/30", icon: "text-purple-400", glow: "shadow-[0_0_30px_rgba(168,85,247,0.18)]" },
};
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        <header>
          <p className="font-mono text-xs uppercase tracking-[0.3em] text-primary">ECU TOOLS</p>
          <h1 className="mt-1 text-3xl font-bold tracking-tight">เครื่องมือ ECU</h1>
        ...
        {!connected && (
          <Card className="border-amber-500/40">
            <CardContent className="flex items-center gap-3 p-4 text-sm">
              ...
              <span>
                ยังไม่ได้เชื่อมต่อ ECM — ไป <a href="/" className="text-primary underline">หน้า Dashboard</a> แล้วกด <span className="font-medium">เชื่อมต่อ</span> ก่อน
              </span>
            </CardContent>
          </Card>
        )}
...
function OpCard({
  ...
  return (
    ...
    >
      ...
      <motion.div
        aria-hidden
        className={cn("pointer-events-none absolute inset-0 rounded-2xl opacity-0 transition group-hover:opacity-100", tone.bg)}
        initial={false}
      />

      {/* Icon badge */}
      <div className={cn("relative flex h-11 w-11 items-center justify-center rounded-xl ring-1", tone.bg, tone.ring)}>
      ...
      <div className="relative mt-1 flex w-full items-center justify-between text-[11px] text-muted-foreground">
        ...
        <ArrowRight className={cn("h-3.5 w-3.5 transition group-hover:translate-x-1", tone.icon)} />
      </div>
    </motion.button>
  );
}
...
function OpModal({ op, onClose }: { op: OpDef | null; onClose: () => void }) {
  ...
  return (
    <Modal
      ...
      width="max-w-3xl"
      accent={op?.tone === "red" ? "ring-red-500/40" : op?.tone === "amber" ? "ring-amber-500/40" : ""}
      title={op && (
        <div className="flex items-center gap-3">
          ...
          <div>
            <p>{op.danger && <span className="mr-1 text-amber-400">⚠</span>}{op.label}</p>
...
function OpModalBody({ def, onClose }: { def: OpDef; onClose: () => void }) {
  ...
  async function run() {
    if (!connected) {
      sound.error();
      toast.error("ยังไม่ได้เชื่อมต่อ ECM", "ไปหน้า Dashboard แล้วกด เชื่อมต่อ ก่อน");
      return;
    }
    if (def.danger) {
    ...
    setReadResult(null);
    setOpResult(null);
    setStoreProgress({ label: "เริ่มต้น…", current: 0, total: 1, percent: 0, ts: Date.now() });
    sound.click();
    ...
    try {
      ...
      const localBackend  = (selected?.backend === "libusb" ? "libusb" : "d2xx") as
        "d2xx" | "libusb";
      const daemonBackend = (selected?.daemon_backend ?? "d2xx") as "d2xx" | "libusb";
      ...
      const useBridge = settings.bridgeEnabled && settings.bridgeUrl.trim().length > 0;
      switch (op.kind) {
        case "read": {
          histKind = "read";
          histFamily = op.variant;
          const r = useBridge
            ? await tauri.readEepromViaBridge(settings.bridgeUrl.trim(), op.variant, idx, daemonBackend)
          ...
          if (r.bytes.length === 0) {
            ...
            sound.success();
            toast.success("อ่าน EEPROM สำเร็จ", `${r.bytes.length} bytes ใน ${(r.duration_ms / 1000).toFixed(1)}s`);
          }
          break;
        }
        case "reset": {
          ...
          setOpResult(r);
          histDuration = r.duration_ms;
          if (r.ok) { sound.success(); toast.success("รีเซ็ตจำนวนการอัดสำเร็จ", "หมุน/กุญแจสตาร์ท ปิด-เปิด 5 วิ"); }
          else      { histStatus = "failed"; histNote = "no ack"; sound.warning(); toast.warning("ECU ไม่ตอบ ack"); }
          break;
        }
        case "format": {
          histKind = "format";
          ...
          if (r.ok) { sound.success(); toast.success(`Format 0x${op.fill.toString(16).toUpperCase()} สำเร็จ`); }
          else      { histStatus = "failed"; sound.error(); toast.error("Format ล้มเหลว"); }
          break;
        }
        case "dump": {
          histKind = "dump";
          histFamily = "Shinden";
          const r = await tauri.dumpRom(op.size, idx, localBackend);
          setOpResult(r);
          histDuration = r.duration_ms;
          ...
          if (r.ok && r.bytes && r.bytes.length > 0) {
            sound.success();
            toast.success(`ROM Dump ${op.size} สำเร็จ`, `${r.bytes.length} bytes`);
          } else {
            histStatus = "failed";
      ...
      setStoreProgress({ label: "เสร็จสิ้น", current: 1, total: 1, percent: 100, ts: Date.now() });
    } catch (e) {
      const raw = String(e);
      setError(raw);
      histStatus = "failed";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={cn(
        ...
        {connected
          ? <>เชื่อมต่อแล้ว · <span className="font-mono">[{selected?.backend.toUpperCase()}] {selected?.description}</span></>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {bytes && bytes.length > 0 && (
          <Button variant="outline" onClick={saveToFile}>
            <Save className="h-4 w-4" /> บันทึก {bytes.length.toLocaleString()}-byte .bin
          </Button>
        )}
        {def.danger && (
          <span className="ml-auto inline-flex items-center gap-1 text-xs text-amber-400">
            <AlertTriangle className="h-3.5 w-3.5" /> destructive — มี dialog ยืนยัน
          </span>
        )}
      </div>
      ...
      {(readResult || opResult) && (
        <div className="space-y-3 rounded-md border border-border/40 bg-muted/20 p-4">
          <div className="grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
            <Stat
              label="Operation"
              value={readResult ? `Read EEPROM (${readResult.family})` : opResult?.label ?? ""}
            />
      ...
      <p className="pt-2 text-center text-[11px] text-muted-foreground">
        กด ESC หรือคลิกพื้นหลังเพื่อปิด · K-Line log streaming อยู่ที่หน้า Dashboard
      </p>
    </div>
  );
}
...
function humaniseError(raw: string): string {
  ...
  if (r.includes("recv") && r.includes("timeout")) return "ECU ไม่ตอบสนอง — ตรวจสอบสาย, สวิตซ์รถ";
  if (r.includes("device not found") || r.includes("ft_open")) return "ไม่พบสาย FTDI";
