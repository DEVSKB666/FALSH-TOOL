  ...
  return (
    <AppShell>
      <div className="space-y-8">
        <header>
          <p className="font-mono text-xs uppercase tracking-[0.3em] text-primary">FILE TOOLS</p>
          <h1 className="mt-1 text-3xl font-bold tracking-tight">เครื่องมือไฟล์</h1>
...
function EcuConverterCard() {
  ...
  async function pickInput() {
    ...
    const filters = mode === "decrypt"
      ? [{ name: "ECU file", extensions: ["ECU", "ecu", "ACG", "acg"] }, { name: "All", extensions: ["*"] }]
      : [{ name: "Binary",   extensions: ["bin", "BIN"] },                  { name: "All", extensions: ["*"] }];
    ...
    <Card>
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-2">
            <Input value={input} onChange={(e) => setInput(e.target.value)} placeholder="เลือกไฟล์…" />
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-2">
            <Input value={output} onChange={(e) => setOutput(e.target.value)} placeholder="เลือกที่บันทึก…" />
        ...
        <div className="flex items-center gap-3">
          ...
          {msg && (
            <span className={msg.startsWith("✔") ? "text-emerald-500" : "text-destructive"}>{msg}</span>
          )}
        </div>
      </CardContent>
    </Card>
...
function XdfBreakerCard() {
  ...
  async function pickFile() {
    ...
    const path = await open({
      multiple: false,
      filters: [{ name: "XDF or ADX", extensions: ["xdf", "adx", "XDF", "ADX"] }],
    });
  ...
  async function runBreak() {
    ...
    if (fileBytes.length < 264 || fileBytes[0] !== 0x05 || fileBytes[1] !== 0x22 || fileBytes[2] !== 0x97) {
      sound.warning();
      toast.warning("ไฟล์นี้ไม่มีรหัสพาสเวิร์ด", "Magic bytes 05 22 97 ไม่ตรง — เลือกไฟล์ที่ติดรหัสผ่าน");
      return;
    }
    ...
    setResult(null);
    try {
      const r = await tauri.breakXdfAdx(fileBytes, filename, password.trim() || undefined);
      setResult(r);
      sound.success();
  ...
  async function saveResult() {
    ...
    if (!isTauri) {
      ...
      a.href = url;
      a.download = filename.replace(/\.(adx|xdf)$/i, ".cleaned.$1") || `cleaned.${result.variant.toLowerCase()}`;
      a.click();
    ...
    const defaultName = filename.replace(/\.(adx|xdf)$/i, "") + `.cleaned.${ext}`;
    const path = await save({
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ปลดรหัสไฟล์ TunerPro `.xdf` / `.adx` — strip <code>{"<openpassword>"}</code> +
          <code className="ml-1">{"<modifypassword>"}</code> + reset <code className="ml-1">{"<flags>"}</code>
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="xdf-pwd">
            Password (เว้นว่าง = ใช้ default <code className="font-mono">$9Ad0$!#*)1QqPow^</code>)
          </Label>
          <div className="flex gap-2">
        ...
        {result && (
          <div className="space-y-3 rounded-md border border-border/40 bg-muted/30 p-4">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              ...
              <Stat label="Open password"    value={result.open_password   || "(ว่าง)"} mono />
              <Stat label="Modify password"  value={result.modify_password || "(ว่าง)"} mono />
            </div>
            <details>
              <summary className="cursor-pointer select-none font-mono text-xs uppercase tracking-widest text-muted-foreground">
                Cleaned XML preview ({result.plaintext.length.toLocaleString()} chars)
              </summary>
