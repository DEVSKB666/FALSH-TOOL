...
import { useSettings, type ThemePreset, type BackgroundPreset } from "@/lib/settings";
...
const THEMES: { id: ThemePreset; label: string; preview: string }[] = [
  ...
  { id: "racing", label: "Racing", preview: "bg-gradient-to-br from-red-600 to-zinc-900" },
...
export default function SettingsPage() {
  ...
  function onLogoFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    fr.onload = () => s.setLogoDataUrl(typeof fr.result === "string" ? fr.result : null);
    ...
    <AppShell>
      <div className="space-y-8">
        <header>
          ...
          <p className="mt-1 text-muted-foreground">ปรับแต่ง brand / theme / background ตามใจ</p>
        </header>

        {/* Brand */}
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Type className="h-5 w-5 text-primary" /> Brand</CardTitle>
            <CardDescription>เปลี่ยนชื่อโปรแกรม + โลโก้ที่แสดงในหัวเรื่องและ splash</CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            <div className="space-y-1.5">
              <Label htmlFor="appname">ชื่อโปรแกรม</Label>
              <Input id="appname" value={s.appName} onChange={(e) => s.setAppName(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label>โลโก้ (PNG / JPG / SVG)</Label>
              <div className="flex items-center gap-4">
                {s.logoDataUrl ? (
                  <img src={s.logoDataUrl} alt="" className="h-14 w-14 rounded-lg object-cover ring-1 ring-border" />
                ) : (
                ...
                <div className="flex-1">
                  <Input type="file" accept="image/*" onChange={onLogoFile} className="cursor-pointer" />
                </div>
                {s.logoDataUrl && (
                  <Button variant="outline" onClick={() => s.setLogoDataUrl(null)}>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-primary" /> Animated Background</CardTitle>
            <CardDescription>พื้นหลังเคลื่อนไหวอย่างนุ่มนวลด้านหลังโปรแกรม</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Eye className="h-5 w-5 text-primary" /> Splash Screen</CardTitle>
            <CardDescription>หน้าจอโหลดก่อนเข้าโปรแกรมทุกครั้ง</CardDescription>
          </CardHeader>
          <CardContent className="flex items-center justify-between">
            <Label htmlFor="splash" className="cursor-pointer">แสดง Splash ตอนเปิดโปรแกรม</Label>
            <Switch id="splash" checked={s.showSplash} onCheckedChange={s.setShowSplash} />
          </CardContent>
        </Card>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ส่งคำสั่ง K-Line ผ่าน <code className="font-mono">loy-bridge.exe</code> ที่รันอยู่บนเครื่องอื่น (notebook + FTDI) ผ่าน WiFi/LAN
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
        {/* URL */}
        <div className="space-y-1.5">
          ...
          <p className="text-[11px] text-muted-foreground">
            รูปแบบ: <code className="font-mono">host:port</code> · ถ้าไม่ใส่ port จะใช้ <code className="font-mono">7878</code> · ตัวอย่าง: <code className="font-mono">192.168.1.50</code> หรือ <code className="font-mono">tcp://10.0.0.5:7878</code>
          </p>
        </div>
        ...
        <div className="flex items-center justify-between rounded-md border border-border/40 bg-background/40 p-3">
          <div>
            <Label htmlFor="bridge-enabled" className="cursor-pointer text-sm font-medium">
              เปิดใช้งาน Bridge
            </Label>
            <p className="mt-0.5 text-[11px] text-muted-foreground">
              เมื่อเปิดแล้ว ทุกการ scan / connect / read EEPROM จะถูกส่งผ่าน bridge แทน FTDI ในเครื่องนี้
            </p>
          </div>
          <Switch
            id="bridge-enabled"
        ...
        {lastErr && (
          <pre className="overflow-x-auto rounded-md border border-red-500/40 bg-red-500/10 p-2 font-mono text-[11px] text-red-200">{lastErr}</pre>
        )}

        <details className="rounded-md border border-border/40 bg-background/40 p-3">
          <summary className="cursor-pointer text-xs text-muted-foreground">วิธีใช้ + คำเตือน</summary>
          <ul className="mt-2 list-disc space-y-1 pl-5 text-[11px] text-muted-foreground">
            <li>รัน <code className="font-mono">loy-bridge.exe --bind 0.0.0.0:7878</code> บน notebook ที่ต่อ FTDI</li>
            <li>ใช้คำสั่ง <code className="font-mono">ipconfig</code> เพื่อหา IP ของ notebook</li>
            <li>กรอก IP:port ในช่อง Bridge URL ด้านบน → กดทดสอบ</li>
            <li>เปิด toggle → ทุกหน้า GUI จะใช้ bridge แทน FTDI ในเครื่องนี้</li>
            <li>⚠ Bridge ไม่มี authentication — bind LAN ที่เชื่อถือเท่านั้น</li>
          </ul>
        </details>
      </CardContent>
    </Card>
