...
export const useConnection = create<ConnectionState>((set, get) => ({
  ...
  async scanPorts() {
    ...
    try {
      ...
      set({
        ...
        selectedKey:
          get().selectedKey ?? (ports[0] ? portKey(ports[0]) : null),
      });
      return ports;
    } catch (e) {
  ...
  async connect(key) {
    const target = key ?? get().selectedKey ?? (get().ports[0] ? portKey(get().ports[0]) : null);
    if (!target) {
    ...
    try {
      ...
      if (found.backend === "bridge") {
        set({ status: "error", error: "bridge port leaked into local-mode connect" });
